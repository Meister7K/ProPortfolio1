...
function Clock() {
  ...
  const compose =
    (...fns: any) =>
    (arg: any) =>
      fns.reduce((composed: any, f: (arg0: any) => any) => f(composed), arg);
  ...
  const display =
    (target: { (time: any): void; (arg0: any): any }) => (time: any) =>
      target(time);

  const formatClock =
    (format: string) =>
    (time: { hours: any; minutes: any; seconds: any; ampm: any }) =>
      format
  ...
  const convertToCivilianTime = (clockTime: any) =>
    compose(appendAMPM, civillianHours)(clockTime);

  const doubleDigits = (civillianTime: any) =>
    compose(
      prependZero("hours"),
      prependZero("minutes"),
      prependZero("seconds")
